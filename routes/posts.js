...
router.get('/', function(req, res, next) {
  db("SELECT posts.id, posts.content, posts.user_id, posts.created_date, posts.image, users.nick, COUNT(CASE WHEN votes.wow = 1 THEN 1 END) AS wow, COUNT(CASE WHEN votes.wow = 0 THEN 1 END) AS meh FROM posts LEFT JOIN votes ON posts.id = votes.post_id INNER JOIN users ON users.id = posts.user_id GROUP BY posts.id ORDER BY posts.id DESC;")
    .then(results => {
